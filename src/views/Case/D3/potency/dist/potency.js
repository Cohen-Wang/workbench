...
export default {
  ...
  // ...
  excellentGroup: null, // 优秀员工组
  generalGroup: null, // 普通员工组
  ...
  tempShow() {
    ...
    // | 最差组
    // +------------------------------------------------------------------------------------------------
    this.excellentGroup = this.groupObj
      .append('g')
      .attr('transform', `translate(${this.width / 2}, ${this.height / 2})`)

    // 背景
    this.excellentGroup.append('circle')
      .attr('r', 900) // 圆圈半径的大小
      .attr('stroke-width', 2)
      .attr('fill', '#FFFFFF')

    // 单个节点的组
    this.excellentSingleNode = this.excellentGroup.selectAll('.circleText')
      .data(this.thirdLevelData)
      .enter()
      .append('g')
      .attr('transform', d => {
    ...
    // 添加点击事件
    this.excellentSingleNode.on('click', d => {
      alert(d.id)
    })

    // 添加circle背景
    this.excellentSingleNode.append('circle')
      .attr('r', 30) // 圆圈半径的大小
      .attr('stroke-width', 1)
      .attr('stroke', 'red')
      .attr('fill', 'yellow')
    ...
    this.excellentSingleNode.append('clipPath')
    ...
    // 绘制图片及其属性
    this.excellentSingleNode.append('image')
      .attr('xlink:href', d => {
        return 'https://wx1.sinaimg.cn/large/006AAcdyly1ghkkr4ll4gj30r50r4wgq.jpg'
      })
      .attr('x', -30)
      .attr('y', -30)
      .attr('width', 60)
    ...
    // 绘制文字及其属性
    this.nodeTest = this.excellentSingleNode.append('text')
      .text(d => d.name)
